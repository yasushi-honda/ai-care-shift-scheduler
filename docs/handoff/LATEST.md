...
**更新日**: 2026-02-15
**フェーズ**: Phase 2 Solver化（ロードマップフェーズ2）本番デプロイ完了
...
### A. Phase 2 Solver版の本番検証（直近1週間）
**目的**: 本番デプロイ完了したSolver機能の動作確認と最適性検証

- [ ] UI から実際のシフト生成実行（Solver版で自動使用）
- [ ] 生成結果の品質確認
  - スコア（Level 1違反 0件が目標）
  - 処理時間（1秒以内）
  - 決定性確認（同一入力で同一出力）
...
**確認方法**:
- アプリUIから「シフト生成」実行
- または: `curl -X POST https://asia-northeast1-ai-care-shift-scheduler.cloudfunctions.net/solverGenerateShift -H "Content-Type: application/json" -d '{...}'`

**関連ファイル**:
- `functions/.env` - SOLVER_FUNCTION_URL 設定済み
- `functions/src/solver-client.ts` - Solver呼び出しロジック（実装済み）
- `solver-functions/main.py` - Solver関数エントリポイント
