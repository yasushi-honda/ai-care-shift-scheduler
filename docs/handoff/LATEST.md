...
**更新日**: 2026-07-25（GCP課金調査セッション・コード変更なし）
**フェーズ**: LLM→Solver完全移行 **本番稼働中** ✅
**最新作業**: GCP課金調査 + クライアント向け説明送付（コード変更なし。直近マージは PR #124）
...
1. **PR #123マージ** (2026-02-25): デモシフトリセット 評価履歴削除漏れ修正
   - Cloud Function `resetDemoShifts` で `schedules` に加え `aiGenerationHistory` も削除
   - `App.tsx` の `onResetComplete` に `setEvaluation(null)` 追加（即時UIクリア）
   - デバッグ用 console.log 除去（DemoBanner.tsx, demoResetService.ts）
   - **根本原因**: リセット後に `getLatestEvaluationForMonth` が旧評価を復元していた

2. **PR #118マージ** (2026-02-20): ヘルプページに管理者機能（CSV一括インポート）セクション追加
   - ヘルプページ（/help）に「管理者機能」セクションを新設
   - CSVインポート手順、テンプレート取得方法、バリデーション仕様を記載

3. **PR #117マージ** (2026-02-20): CSV一括インポート機能
...
- **Solver**: 68/68テスト通過
- **Frontend**: 348テスト通過（PR #117 で32件追加後の全数）
- **Backend**: 230テスト通過
...
2. **4段階評価システム**: Level 1-4の制約評価
   - Level 1（労基法）: 即0点 / Level 2（人員不足）: -5点/件
