...
| シフト | 必要人数/日 | このバッチ目標 |
|--------|-------------|---------------|
| 早番 | ${earlyCount}名 | ${Math.max(1, Math.round(earlyCount * staffBatch.length / totalStaffCount))}名/日 |
| 日勤 | ${dayCount}名 | ${Math.max(1, Math.round(dayCount * staffBatch.length / totalStaffCount))}名/日 |
| 遅番 | ${lateCount}名 | ${Math.max(1, Math.round(lateCount * staffBatch.length / totalStaffCount))}名/日 |

⚠️ **日勤偏重に注意**: 日勤に偏りやすいので、**早番を優先的に配置**してください。
早番・日勤・遅番の比率は ${earlyCount}:${dayCount}:${lateCount} を目指してください。
${dynamicConstraints}
# 制約
...
□ **早番が日勤より少なくないか**（早番:日勤 = ${earlyCount}:${dayCount}の比率を確認）
□ 遅番が適切に配置されているか
