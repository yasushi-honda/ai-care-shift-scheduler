...
**AIシフト自動作成**は、介護・福祉事業所のシフト作成業務を劇的に効率化するWebアプリケーションです。Google の最新AI「Gemini 2.5 Flash」を活用し、スタッフの資格・希望・労働基準法を考慮した最適なシフトを自動生成します。
...
- ✅ **AIシフト自動生成**: Gemini 2.5 Flash AIによる最適化
- ✅ **カレンダー表示**: 直感的なシフト確認
...
**問題**: `Model gemini-2.5-flash not found`
```bash
...
- 本番環境での実行は `--force` オプションが必須です（公開前のテスト目的のみ使用）
- 公開後は開発環境とステージング環境を分けて運用することを推奨します
