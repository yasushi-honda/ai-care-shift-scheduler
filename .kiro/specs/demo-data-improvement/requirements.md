...
- [x] 実装完了（第3回修正）
- [x] デイサービス実態調査完了
- [x] Firestore検証完了（第4回修正: 削除ロジック追加）
- [ ] 実機テスト完了（AIシフト生成の動作確認）

---

## 9. 追加修正: 削除ロジックバグ（第4回修正）

### 9.1 発見された問題

`seedDemoData.ts`の`--reset`オプションが**削除処理を実行していなかった**バグを発見。

- **症状**: 「削除します」のログは出力されるが、実際にはFirestoreからデータが削除されない
- **結果**: 古いスタッフ（夜勤専従など）が残存し、新規スタッフが追加されて11名になっていた

### 9.2 修正内容

`seedDemoData.ts`に実際の削除ロジックを追加:
...
`scripts/verifyDemoData.ts`を作成し、Firestoreのデモデータを検証:
...
| 2025-12-07 | Claude | 第4回修正（seedDemoData.ts削除ロジックバグ修正、検証ツール作成） |
