...
**使用方針**:
- バージョン固定なし: `gemini-2.5-flash-lite` は自動的に最新の安定版（GA版）を使用
- 本番環境対応: GA版のため、本番環境での使用に適している
- Preview版との違い:
  - ❌ 使用しない: `gemini-flash-lite-latest` （Preview版エイリアス）
  - ❌ 使用しない: `gemini-2.5-flash-lite-preview-09-2025` （具体的なPreview版）
  - ✅ 使用する: `gemini-2.5-flash-lite` （GA版エイリアス）
