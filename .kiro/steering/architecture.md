...
- **モデル命名の正確な理解**:
  - ✅ `gemini-2.5-flash-lite` - GA版エイリアス（本番環境推奨）
  - ⚠️ `gemini-flash-lite-latest` - Preview版エイリアス（存在するが本番非推奨）
  - ℹ️ `gemini-2.5-flash-lite-preview-09-2025` - 具体的なPreview版（2週間前廃止通知）
  - ❌ `gemini-2.5-flash-lite-latest` - このような命名形式は存在しない
...
try {
  ...
  if (error.code === 'RESOURCE_EXHAUSTED') {
    ...
    // その他のエラー
    return { error: 'シフト生成に失敗しました。デモシフト作成ボタンをお試しください。' };
  }
}
```
